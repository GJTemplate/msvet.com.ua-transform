...
class VirtueMartCustomFieldRenderer {
	...
	static function getMediaCustomFields ($objMultyVariant){
		...
		$SUBquery
			->select( $db->quoteName(  'customs.virtuemart_custom_id' )  )  
			->from($db->quoteName('#__virtuemart_customs' , 'customs' ) ) 
			->where($db->quoteName('layout_pos') .' = '.$db->quote('PropertiesProduct') ) ;
			
		 
		$query
			->select('c.virtuemart_custom_id')
			->select('c.layout_pos')
			
			 
			->from($db->quoteName('#__virtuemart_customs' , 'c') ) 
			->where($db->quoteName('custom_parent_id') . ' IN (' . $SUBquery . ')' ) 
			
			->where($db->quoteName('field_type') . ' LIKE  ' . $db->quote( 'S' )  ) 
	...
	static function renderCustomfieldsFE(&$product,&$customfields,$virtuemart_category_id){
		...
		foreach($customfields as $k => $customfield){
			...
			switch ($type) {
				...
				case 'C': // Поле выбора свойств товара
					//  позиции дополнительных полей выбора свойст
					//  
					//  
					//  ID - цвета товара  - ColorSelectionID
					//  
					...
					//echo '<pre>'; print_r ( $customfield); echo '</pre>'.__FILE__.'Строка '.__LINE__ ;
					...
					foreach($customfield->selectoptions as $k => $soption){
						...
						if(isset($dropdowns[$k])){
							foreach($dropdowns[$k] as $i=> $elem){
								...
								if($productSelection   ){
									$productSelection[$k] = preg_replace('|[\s]+|s', ' ', $productSelection[$k]);
									$elem = preg_replace('|[\s]+|s', ' ', $elem);
									 
									$variants['0'] = htmlentities($variants['0']);
									
									$arr1 = str_split($productSelection[$k]);
									$arr2 = str_split($elem);
									
									if( $productSelection[$k] == $elem ){
										$selected = $elem;
									} // end if
								} // end if
							} //foreach
						...
						$attribs = array(
							'class'=>$class.' cvselection no-vm-bind',
					...
					/*	if('productdetails' != $view || !VmConfig::get ('jdynupdate', TRUE)){
							$attribs['reload'] = '1';
							$view = 'productdetails';
						}*/
					...
					foreach($customfield->options as $product_id=>$variants){
						$variants['0'] = str_replace('  ', ' ', $variants['0']);
					...
					// $MediaObj = self::getMediaCustomFields ( $customfield->options  );
					...
					foreach ( $customfield->options as  $k => $v ){
						$prod  = $productModel->getProduct($k);
						$productModel->addImages($prod );					 
						
						if( !$prod->virtuemart_product_id  ){continue;} // end if
						
					 	$mediaArr[$i]['virtuemart_product_id'] = $prod->virtuemart_product_id;
						$mediaArr[$i]['product_name'] = $prod->product_name;
						$mediaArr[$i]['product_sku'] = $prod->product_sku;
						$mediaArr[$i]['prices'] = $prod->prices['product_price'];
						
						$mediaArr[$i]['product_s_desc'] = $prod->product_s_desc;
						$mediaArr[$i]['file_url'] = $prod->file_url; 
		...
		//			json_encode ( $mediaArr);
					
					if($customfield->virtuemart_custom_id == 51 ){
						/*echo '<pre>'; print_r ( $customfield ->options   ); echo '</pre>'.__FILE__.' in line:  '.__LINE__ ;
						die(__FILE__.' in line '.__LINE__);*/
						
						
						 $html .= shopFunctionsF::renderVmSubLayout('customfield_colorFilters',array(
						 	'customfield'=>$customfield, 
							'jsArray'=>$jsArray, 
							'mediaArr'=>$mediaArr, 
						 	'tag' => "Virtuemart.SelectorVariants = { variants:[".$jsVariants."] , media:". json_encode ( $mediaArr)." } " ,
						 	'hesh' => $hash,
						));	
					} // end if
					...
					var ".$tags[0]." = { variants:[".$jsVariants."] , media:". json_encode ( $mediaArr)." }
					function getSelectorVariants() { return  ".$tags[0]." ; }
					...
					//  vmJsApi::addJScript('cvselvars'.$hash,$j,false,true,false,false);

					$doc = JFactory::getDocument();
					$doc->addScriptDeclaration( $j );


					//Now we need just the JS to reload the correct product  '/*. json_encode ( $mediaArr).*/'
