 ...
 * @version $Id: default_showprices.php 7499 2013-12-18 15:11:51Z Milbo $
 */
// Check to ensure this file is included in Joomla!
defined ('_JEXEC') or die('Restricted access');
?>
	<div class="discount_product_wrap">
		<?php if(!empty($this->product->prices['discountAmount'])){ 
			$percent = round(-100*$this->product->prices['discountAmount']/$this->product->prices['priceWithoutTax'] ); 
			echo '<div class="percentPrice">
					<!--div class="discount_product_title">На данный товар скидка:</div-->
					<div class="discount_product_data">' .$percent.'<span>%</span></div>
				  </div>'; 
		} ?> 
    </div>  
<div class="product-price" id="productPrice<?php echo $this->product->virtuemart_product_id ?>">
	...
	if ($this->product->prices['salesPrice']<=0 and VmConfig::get ('askprice', 1) and isset($this->product->images[0]) and !$this->product->images[0]->file_is_downloadable) {
        ...
        <a class="ask-a-question bold" href="<?php echo $this->askquestion_url ?>" rel="nofollow" ><?php echo vmText::_ ('COM_VIRTUEMART_PRODUCT_ASKPRICE') ?></a>
		<?php
	} else {
