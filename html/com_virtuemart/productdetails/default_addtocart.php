	...
	<form method="post" class="product js-recalculate" action="<?php echo JRoute::_ ('index.php',false); ?>">
		...
		*/
		...
		if (!empty($this->product->customfieldsSorted[$this->position])) {
         ...
         <div class="FiltersColoringWraper">
                <?php
				// Вывод фильтра для цветов и пр.
				
				if( isset ($this->product->customfieldsSorted[$this->position] ) ){
 		...
 		return true;
			...
			if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($this->product->product_in_stock - $this->product->product_ordered) < 1) {
				...
				<?php // Display the quantity box END
					...
					$addtocart_button = shopFunctionsF::getAddToCartButton ( $this->product->orderable   );
					
					
					$v ='<input';
					$s = '<input onClick="ga(\'send\', \'event\', \'Buy\', \'Submit\');" ';
					//$s = '<input ';
					
					 
					
					$addtocart_button =  str_replace($v, $s, $addtocart_button);
					echo $addtocart_button;
