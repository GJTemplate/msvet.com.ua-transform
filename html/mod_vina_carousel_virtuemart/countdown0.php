	...
	<div class="item vinavm-product vm-col round-corners">
		<div class="ma-box-content">
			...
			<div class="text-block">
				<!-- Product Name -->
				<?php if($productName) : ?>			
				<h3 class="product-title"><a href="<?php echo $pLink; ?>" title="<?php echo $pName; ?>"><?php echo mb_substr( $product->product_name, 0, 17); ?></a></h3>
				<?php endif; ?>
...
jQuery(document).ready(function($) {
	$('[data-countdown]').each(function() {
		...
		$this.countdown(finalDate, function(event) {
			$this.html(event.strftime('<div class="day">До конца акции осталось: <span class="number">%D</span>дней</div> <div class="hour"><span class="number">%H </span>часов</div><div class="min"><span class="number"> %M</span> минут</div> <div class="sec"><span class="number">%S </span>секунд</div>'));
		});
	});
});
</script>
